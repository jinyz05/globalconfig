...
HTTPManager::HTTPManager(QObject *parent) :
        QObject(parent),mTimer1(this)
{    
    mEDPManager=NULL;
    mbSentAddStream=false;
    mHttpClient=new HTTPClient(this);
    Login();
    connect(&mTimer1,SIGNAL(timeout()),this,SLOT(GetDeviceDataPoint()));
    mTimer1.setSingleShot(true);
    mTimer1.start(60000);
}
void HTTPManager::Login()
{
    mHttpClient->httpLogin();
}
void HTTPManager::ProductKeyAndID(QString devicename,QString &deviceid,QByteArray& key)
{
...
void HTTPManager::ProductMasterKey(QString devicename,QByteArray& key)
{
    if(devicename=="C240")
    {
        key=C240_MASTER_KEY;
...
void HTTPManager::AddDeviceStream(QString device_name)
{
    QString sn=SysInfoModel::Instance()->GetSysSetting().serialno;
    if(sn.isEmpty())
        return;
    if(mbSentAddStream)
        return;
...
void HTTPManager::AddDeviceStreamResp()
{
...
void HTTPManager::GetDeviceStream(QString sn,QString device_id,QByteArray key)
{
    if(mDataPointReply!=NULL)
    {
        mDataPointReply->abort();
...
void HTTPManager::GetDeviceStreamResp()
{
    printf("%s,%s,%d:%s\n",__FILE__,__func__,__LINE__,"");
    QNetworkReply* reply=qobject_cast<QNetworkReply*>(sender());
    if(reply&&mDataPointReply)
    {
        ...
        if(ret==1)
        {
            ...
            if(t.size()==3)
            {
                ...
                QByteArray id=t.at(1);
                QByteArray key=t.at(2);
...
void HTTPManager::GetDeviceDataPoint()
{
    if(mDeviceID.isEmpty()||mDeviceKey.isEmpty())
    {
        QString sn=SysInfoModel::Instance()->GetSysSetting().serialno;
        if(sn.isEmpty())
            return;
        ...
        QString device_id;
        QByteArray key;
        ProductKeyAndID(device_id,key);
        GetDeviceStream(sn,device_id,key);
        mTimer1.start(60000);
    }
    else
    {
        if(mEDPManager==NULL)
            mEDPManager=new EDPManager(this,mDeviceID,mDeviceKey);
    }
}
void HTTPManager::GetDeviceDatapoint(QString sn,QString device_id,QByteArray key)
{    
    if(mDataPointReply!=NULL)
    {
        mDataPointReply->abort();
    ...
    QString path="http://api.heclouds.com/devices/"+device_id;
    path+="/datapoints?datastream_id=";
    path+=sn;
    QNetworkReply* reply=mHttpClient->httpGet(QUrl(path),key);
    mDataPointReply=reply;
    connect(reply,SIGNAL(finished()),this,SLOT(GetDeviceDatapointResp()));
}
int HTTPManager::ParseDeviceDatapoint(QByteArray data,QByteArray& datapointvalue)
{
    int ret=0;
    printf("data:%s\n",data.constData());
    cJSON *errnoJSON=NULL;
    ...
    if(errnoJSON && errnoJSON->valueint==0)
    {
        ...
        if(data)
        {
            cJSON* datastreams=cJSON_GetObjectItem(data,"datastreams");
            if(datastreams)
            {
                int size=cJSON_GetArraySize(datastreams);
                if(size>0)
                {
                    ...
                    if(datastream)
                    {
                        ...
                        if(datapoints)
                        {
                            int size=cJSON_GetArraySize(datapoints);
                            if(size>0)
                            {
                                ...
                                if(datapoint)
                                {
                                    ...
                                    if(value)
                                    {
                                        datapointvalue = QByteArray(value->valuestring);
                                        ret=1;
                                        if(root)
                                            cJSON_Delete(root);
                                        return ret;
...
int HTTPManager::ParseDeviceStream(QByteArray data,QByteArray& value)
{
    int ret=0;
    printf("data:%s\n",data.constData());
    cJSON *errnoJSON=NULL;
...
void HTTPManager::DecodeDeviceValue(QByteArray & name_,QByteArray& id_,QByteArray& key_)
{
...
void HTTPManager::GetDeviceDatapointResp()
{
    printf("%s,%s,%d:%s\n",__FILE__,__func__,__LINE__,"");
    QNetworkReply* reply=qobject_cast<QNetworkReply*>(sender());
    if(reply&&mDataPointReply)
    {
        ...
        reply->deleteLater();
        mDataPointReply=NULL;
        QByteArray datapointvalue;
        int ret=ParseDeviceDatapoint(data,datapointvalue);
        if(ret==1)
        {            
...
void HTTPManager::AddDevice(QString devicename,QByteArray device_sn)
{
...
void HTTPManager::AddDeviceResp()
{
    bool ok=false;
...
void HTTPManager::DeleteDevice(QString devicename,QString device_id)
{
    QByteArray masterkey;
    QString path="http://api.heclouds.com/devices/"+device_id;
    ProductMasterKey(devicename,masterkey);
...
void HTTPManager::DeleteDeviceResp()
{
    bool ok=false;
    printf("%s,%s,%d:%s\n",__FILE__,__func__,__LINE__,"");
    QNetworkReply* reply=qobject_cast<QNetworkReply*>(sender());
...
void HTTPManager::AddDeviceKey(QString devicename,QByteArray device_id,QByteArray device_sn)
{
    QByteArray masterkey;
...
void HTTPManager::AddDeviceKeyResp()
{
    bool ok=false;
...
void HTTPManager::DeleteDeviceKey(QString devicename,QByteArray key)
{
...
void HTTPManager::DeleteDeviceKeyResp()
{
    bool ok=false;
    printf("%s,%s,%d:%s\n",__FILE__,__func__,__LINE__,"");
    QNetworkReply* reply=qobject_cast<QNetworkReply*>(sender());
...
void HTTPManager::AddDeviceDatapoint(QString devicename,QByteArray device_key,QByteArray device_sn,QByteArray device_id)
{
    QString id;
    QByteArray key;
...
void HTTPManager::AddDeviceDatapointResp()
{
    bool ok=false;
    printf("%s,%s,%d:%s\n",__FILE__,__func__,__LINE__,"");
    QNetworkReply* reply=qobject_cast<QNetworkReply*>(sender());
...
void HTTPManager::DeleteDeviceDatapoint(QString devicename,QByteArray device_sn)
{
    QString id;
    QByteArray key;
...
void HTTPManager::DeleteDeviceDatapointResp()
{
    bool ok=false;
    printf("%s,%s,%d:%s\n",__FILE__,__func__,__LINE__,"");
    QNetworkReply* reply=qobject_cast<QNetworkReply*>(sender());
...
void HTTPManager::GetDeviceStreamList(QString devicename,QByteArray key)
{
    QString device_id;
...
void HTTPManager::GetDeviceStreamListResp()
{
    printf("%s,%s,%d:%s\n",__FILE__,__func__,__LINE__,"");
    QNetworkReply* reply=qobject_cast<QNetworkReply*>(sender());
    if(reply&&mDataPointReply)
...
int HTTPManager::ParseDevicesStreams(QByteArray data)
{
    int ret=0;
    printf("data:%s\n",data.constData());
    cJSON *errnoJSON=NULL;
